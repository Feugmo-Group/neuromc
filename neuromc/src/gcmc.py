...
# Optional dft module for external-field and density-grid support
try:
    from neuromc.dft.external_field import ExternalField
...
class MLP_GCMC:
    ...
    def __init__(self, model, atoms_frame, atoms_ads, T, P, fugacity, device, vdw_radii, debug=False, output_dir='results', restart_prefix=None, n_equilibration_steps=None, n_production_steps=None, checkpoint_interval=10000, write_trajectory=False, trajectory_interval=100, overwrite_checkpoints=False, external_field=None, density_grid=None):
        ...
        # cDFT data-generation extensions
        self.external_field = external_field      # Optional[ExternalField]
        self.density_grid = density_grid          # Optional[DensityGrid1D | DensityGrid3D]
        ...
        self.log_file_path = Path(self.output_dir) / f"log_{self.P/bar:.4f}bar.bin"


    def _external_field_energy(self, atoms) -> float:
        """Sum V_ext over adsorbate-atom positions (framework contribution cancels in ΔE)."""
        if self.external_field is None or self.Z_ads == 0:
            return 0.0
        ads_positions = atoms.get_positions()[self.n_frame:]
        return float(self.external_field(ads_positions).sum())

    def _accumulate_density(self, atoms, step: int, n_equilibration_steps: int, steps_per_block: int) -> None:
        """Add current frame to the density grid during production."""
        if self.density_grid is None or self.Z_ads == 0:
            return
        if step <= n_equilibration_steps:
            return
        ads_positions = atoms.get_positions()[self.n_frame:]
        self.density_grid.accumulate(ads_positions, steps_per_block)
    ...
    def _write_trajectory_xyz(self, atoms: Atoms) -> None:
        ...
        traj_file = Path(self.output_dir) / f'traj_{self.P/bar:.4f}bar.xyz'
    ...
    def _get_restart_paths(self) -> Tuple[str, str]:
        ...
        restart_xyz = os.path.join(restart_dir, f'restart_{self.P/bar:.4f}bar.xyz')
        restart_json = os.path.join(restart_dir, f'restart_{self.P/bar:.4f}bar.json')
    ...
    def _save_history_checkpoint(self, atoms: Atoms, step: int, uptake: int, interaction_energy: float, total_energy: float, overwrite_checkpoints: bool = False) -> None:
        ...
        checkpoints_parent_dir = Path(self.output_dir) / f'checkpoints_{self.P/bar:.4f}bar'
    ...
    def _save_results_json(
        ...
        filename = Path(self.output_dir) / f"results_{self.P/bar:.4f}bar.json"
    ...
    def run(self, N: int) -> None:
        ...
        results_file = Path(self.output_dir) / f"results_{self.P/bar:.4f}bar.json"
        ...
        for iteration in range(steps_to_run):
            ...
            if switch < MOVE_PROBABILITIES['insertion']:
                ...
                # Add external-field contribution to the free-energy difference
                vext_initial = self._external_field_energy(atoms)
                vext_final = self._external_field_energy(atoms_trial)
                initial_int_E += vext_initial
                final_int_E += vext_final
            ...
            # Translation
                ...
                if self.Z_ads > 0:
                    ...
                    if vdw_overlap(atoms_trial, self.vdw, self.n_frame, self.n_ads, i_ads):
                        ...
                        atoms_trial.calc = self.model
                        e_trial = atoms_trial.get_potential_energy()
                    vext_delta = (self._external_field_energy(atoms_trial)
                                  - self._external_field_energy(atoms))
                    acc = min(1, np.exp(-self.beta * (e_trial - e + vext_delta)))
            ...
            # Density accumulation for cDFT data generation (production phase only)
            n_equil = self.n_equilibration_steps or 0
            steps_per_block = max(1, (self.n_production_steps or N) // max(1, getattr(self.density_grid, 'n_blocks', 1)))
            self._accumulate_density(atoms, current_step, n_equil, steps_per_block)

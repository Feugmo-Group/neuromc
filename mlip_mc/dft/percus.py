...
def c1_percus(rho: np.ndarray, xs: np.ndarray, sigma: float = 1.0) -> np.ndarray:
    """
    Compute c₁(x) = ln(1 − n₁(x)) from Percus' exact 1D hard-rod functional.

    The weighted density n₁(x) = ∫_{x-R}^{x+R} ρ(x') dx' is the local
    packing fraction in the sphere of radius R = σ/2 around x.

    This gives the exact Tonks bulk EOS:
        μ = ln(ρ/(1−ρ))  →  c₁_bulk = ln(1−ρ) < 0
    ...
    """
    dx = xs[1] - xs[0]
    _, omega1 = percus_weights(xs, sigma=sigma)

    n1 = _fft_conv(omega1, rho, dx)
    n1 = np.clip(n1, 0.0, 1.0 - 1e-12)

    return np.log(1.0 - n1)

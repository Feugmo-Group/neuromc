...
class HardRodSystem:
    ...
    def sweep(self, n_transitions: int = 10, insert_delete_prob: float = 0.2) -> None:
        """
        Perform n_transitions MC attempts per particle (at least 1).

        insert_delete_prob : fraction of attempts that are insertion/deletion.
        Remaining are translation moves.
        """
        n_attempts = max(1, self.n_particles) * n_transitions
...
def simulate(
    ...
    for _ in range(n_equil):
        system.sweep(n_transitions=10, insert_delete_prob=0.3)
